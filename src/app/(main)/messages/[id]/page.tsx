...
export default function ConversationPage() {
  ...
  if (error) {
    return (
      <div className="min-h-dvh bg-background flex flex-col items-center justify-center p-6">
        ...
        <button onClick={loadData} className="px-4 py-2 bg-primary text-white rounded">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-dvh bg-background flex flex-col">
      {/* Header */}
      <header className="px-4 pt-4 pb-4 border-b border-divider flex items-center justify-between sticky top-0 bg-background z-10">
        <div className="flex items-center gap-3">
          <Link href="/messages" className="w-10 h-10 border border-divider rounded-full flex items-center justify-center text-primary hover:bg-surface transition-colors">
          ...
          <div>
            ...
            <p className="text-xs text-foreground-secondary">Re: {conversation?.item.title}</p>
          </div>
        </div>
        {!isHandoverComplete && (
          <button
        ...
        <div className="px-4 py-3 bg-surface border-b border-divider">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="px-4 py-2 bg-primary text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
            >
              Verify Now
            </button>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 scroll-smooth">
        {messages.length > 0 && (
          <div className="flex justify-center mb-6">
            <span className="px-4 py-1.5 bg-surface border border-divider rounded-full text-xs font-medium text-foreground-secondary">
        ...
        <div className="space-y-4 pb-4">
          ...
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {!isMine && conversation && (
                  <div className="w-8 h-8 bg-surface rounded-full flex items-center justify-center shrink-0 overflow-hidden border border-divider">
                ...
                >
                  ...
                  <p className={`text-[10px] mt-1 ${isMine ? 'text-white/70' : 'text-foreground-muted'} text-right`}>
      ...
      {/* Input Area */}
      <div className="px-4 py-3 border-t border-divider safe-area-bottom bg-background sticky bottom-0">
        <div className="flex items-center gap-3 max-w-lg mx-auto w-full">
          <input
            ...
            className="flex-1 h-12 pl-4 pr-4 bg-surface rounded-full text-sm text-foreground placeholder:text-foreground-muted focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all border border-divider focus:border-primary"
          />
          <button
            onClick={handleSend}
            disabled={!message.trim()}
            className="w-12 h-12 bg-primary rounded-full flex items-center justify-center shrink-0 transition-all hover:shadow-lg active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          >
